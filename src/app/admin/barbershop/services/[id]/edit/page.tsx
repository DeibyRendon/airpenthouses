...
export default async function EditBarberServicePage({ params }: { readonly params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-800 pb-20">
      <nav className="w-full bg-slate-900 text-white p-4 shadow-md flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold font-sans tracking-tight">Editar Servicio: {service.name}</h1>
      </nav>
      ...
      <div className="max-w-4xl mx-auto p-6 mt-12 bg-white/50 backdrop-blur-sm rounded-[2rem] border border-slate-100 shadow-sm p-10">
        <div className="mb-10 text-center">
            <h2 className="text-2xl font-black text-slate-900">Actualizar Experiencia</h2>
            <p className="text-slate-500 mt-2">Ajuste los precios, detalles o duración de este servicio de barbería.</p>
        </div>
        <BarberServiceForm initialData={service} />
      </div>
    </main>
  );
}
